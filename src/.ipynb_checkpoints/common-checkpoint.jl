...
function get_factor_graph(num_var, max_order)
    interactions = SortedDict{Tuple, Float64}()
    terms = []
    
    # generate unique combinations of indices
    for x in 2:max_order
        tmp_combinations = unique(
        k -> Set(k), Combinatorics.with_replacement_combinations(collect(1:num_var), x))
        tup_combinations = [tuple(x...) for x in tmp_combinations]
        push!(terms, tup_combinations)
    end

    fg_indices = collect(Iterators.flatten(terms))

    for idx in fg_indices
        interactions[idx] = rand(1)[1]
    end
